    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 m-auto items-start p-8 py-12 w-80 sm:w-[352px] text-gray-500 rounded-lg shadow-xl border border-gray-200 bg-white">
            ...
            <button
                type='submit'
                disabled={loading}
                className={`bg-purple-700 hover:bg-purple-800 transition-all text-white w-full py-2 rounded-md cursor-pointer flex justify-center items-center ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
                {loading ? (
